...
// Helper function to extract flight data from message content
export const extractFlightData = (content: string, courseDetails: any) => {
  ...
  if (content.includes("flight")) {
    ...
    const courseStart = new Date(courseDetails.startDate)
    ...
    const courseEnd = new Date(courseDetails.endDate)
...
export const processAIMessage = (message: any) => {
  const updates = {
    departureFlight: null as Flight | null,
    returnFlight: null as Flight | null,
    hotel: null as HotelType | null,
    entertainment: null as Entertainment | null,
